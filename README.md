...
- **A stable news URL** — the daily news section is retitled `edit. DD.MM.YY` every morning, so its address never changes: it is served from `/` (declared canonical, and the only news URL in the sitemap and feed), with the stable `/news` slug as an alias and the date kept in the title; links to older dated URLs (`/edit-170926`) forward to it
